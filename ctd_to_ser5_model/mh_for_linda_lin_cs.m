...
%%
load best_simple_pars_cs
par_changed = [1,3:7,9:14];
par_fixed = parameters;

par_opt = log10(parameters(par_changed));
get_err = @(pars)-2*sum(get_log_l_simplified_cs(10.^pars,mrna,ser5,rnap,mrna_rnap,mrna_ser5,ser5_rnap,par_fixed,par_changed));
...
for i=1:10
    x0 = par_opt;
    sv_file = ['met_hast_pars_2x_cs_',num2str(i)];
    mh_smpl=[x0];
