...
% 
% n = 30;
% rnap_ser5_pt1 = mean(rnap_ser5_submin(1:n));
% rnap_ser5_pt2 = mean(rnap_ser5_submin(600-floor(n/2):600+floor(n/2) ));
% rnap_ser5_pt3 = mean(rnap_ser5_submin(end-n:end));
% 
% f = @(x) rescale([rnap_ser5_pt1, rnap_ser5_pt2, rnap_ser5_pt3], x(1), x(2)) + x(3);
% f_sse = @(x) sum(   rescale( [rnap_ser5_pt1, rnap_ser5_pt2, rnap_ser5_pt3], x(1),x(2)) + x(3)   - ser5_rnap.mn_cc(10:12)' )^2 ;
% 
% f_sse2 = @(x) sum(   rescale( [rnap_ser5_pt1, rnap_ser5_pt2, rnap_ser5_pt3], x(1),x(2))   - ser5_rnap.mn_cc(10:12)' )^2 ;
% 
% 
% rnap_ser5_submin_data = rnap_ser5_submin(1:60:end);
% 
% rnap_ser5_submin_std = sum( (rnap_ser5_submin - sub_min_data(:,2) ).^2) / length(sub_min_data(:,2));
% 

%%

% TT = [-10:10];
% TTs = linspace(-1,1,1198);
% TTc  = [TT(1:10) -1:1200:1 TT(12:end)];
% plot(TT,ser5_rnap.mn_cc,'o-'); hold on;
% plot(TTs, rescale(rnap_ser5_submin,.66,1)   ,'o')
% 
% 
...
parameters = [parameters(1:3) 100 parameters(4:end)];

par_fixed = parameters;
par_changed = [1,3:7,9:14];
par_opt = log10(par_fixed(par_changed));
get_err = @(pars)sum(get_log_l_simplified_cs(10.^pars,mrna,ser5,rnap,mrna_rnap,mrna_ser5,ser5_rnap,par_fixed,par_changed));
min_err=inf;
%%
for i=1:20
        try
    ...
    err = get_log_l_simplified_cs(10.^par_opt,mrna,ser5,rnap,mrna_rnap,mrna_ser5,ser5_rnap,par_fixed,par_changed);
    if sum(err)<min_err
        min_err=sum(err)
        parameters(par_changed) = 10.^par_opt;
        save best_simple_pars_cs parameters
