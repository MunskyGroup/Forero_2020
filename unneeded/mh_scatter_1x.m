...
cd ./Model_fits/simple_MH_results/3.17.2021/
...
while ikeep==1
    try
        fn = ['met_hast_pars_1x_',num2str(i),'.mat'];
        load(fn)
...
%         scatter(mh_pars(1:100:end,i),mh_pars(1:100:end,j),sz(1:100:end),mh_vals(1:100:end),'filled','MarkerFaceAlpha',.2,'MarkerEdgeAlpha',.2,'MarkerFaceColor',[1,.5,0]); hold on
%         
...
for i=1:Np
    ...
    for j=i+1:Np
        subplot(Np,Np,(j-1)*Np+i)
        scatter(mh_pars(mh_vals(1:100:end) > k,i) ,mh_pars(mh_vals(1:100:end) > k,j),sz(mh_vals(1:100:end) > k),mh_vals(mh_vals(1:100:end) > k),'filled'); hold on
