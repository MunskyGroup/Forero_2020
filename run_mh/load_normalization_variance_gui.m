function [mrna,ser5,rnap,mrna_rnap,mrna_ser5,ser5_rnap,G1_rnap,G1_ser5,G1_mrna] = load_normalization_variance_gui(rezero_acc,norm_type)
...
for j = 1:20
    ...
    C(1:round(length(C)/2)-1,:) = C(1:round(length(C)/2)-1,:)./(1:1:Nt-1)'; 
        ...
        [G1s_rnap(j),G1s_ser5(j),G1s_mrna(j)] = get_g0(mod_rnap_100,mod_ser5_100,mod_mrna_100,norm_type);
end
...
%  plot(mean(mrna_rnap100,2),'o')
 ...
 mrna_rnap.sem_cc = std(mrna_rnap100,0,2)./sqrt(20);
 
 mrna_ser5.mn_cc = mean(mrna_ser5100,2);
 mrna_ser5.sem_cc = std(mrna_ser5100,0,2)./sqrt(20);
 
 ser5_rnap.mn_cc = mean(ser5_rnap100,2);
 ser5_rnap.sem_cc = std(ser5_rnap100,0,2)./sqrt(20);
