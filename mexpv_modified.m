...
function [w, err, hump, Time_array_out, P_array, P_lost, tryagain] = mexpv_modified( t, A, v, tol, m, N_prt, Time_array,eps,SINKS)
...
%             H(i,j) = sum(p.*V(:,i));  %12.091
    ...
    while ireject <= mxrej,
        ...
        if err_loc <= delta * t_step*tol,
            ...
            if ireject == mxrej,
                ...
                tryagain=1;
                P_lost=w(SINKS);
                %             time_of_excess = t_now/t;
                w=[];
                err=[];
                hump=[];
                Time_array_out=Time_array;
                P_array=[];
                %%
                return
    ...
    if t_now>=Time_array(i_prt)
        i_prt=i_prt+1;
        Time_array_out = [Time_array_out,t_now];
        P_array = [P_array,w];
    end
    %%
